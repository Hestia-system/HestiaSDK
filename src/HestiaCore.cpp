...
namespace HestiaCore {
     ...
     *
     *  Notes:
     *    • This call only stores pointers; it does not perform any instantiation.
     *      RegisterEntitiesIotBridge() still handles the creation of HAIoTBridge objects.
     *
     *    • This enables dependency injection as required by Architecture S-2.
     ...
     *
     *    • The table may reside in PROGMEM. Only the pointer and count are retained.
     */
    ...
    // =====================================================================================
    //  logSummary() — Diagnostic summary of all registered entities
    // =====================================================================================
    void logSummary() {
    ...
    // =====================================================================================
    //  InitValueNVS() — Restore values from NVS through each bridge object
    // =====================================================================================
    /**
     ...
     *   • Each entity decides whether to restore from NVS or use default metadata.
     */
    bool InitValueNVS() {
    ...
    // =====================================================================================
    //  publishValuesToHA() — Publish values to HA through each bridge object
    // =====================================================================================
    /**
     ...
     *    • The internal static bridge_config[] array is no longer referenced.
     *    • Instead, the firmware (or external tools) injects a BridgeConfig table
     *      via loadBridgeConfig().
     *
     *  Benefits:
     *    • Device-agnostic SDK
     *    • Supports very large tables stored in PROGMEM
     *    • Allows dynamic configuration generation
     *
     *  Constraints:
     *    • Behavior is identical to R1/R0 unless loadBridgeConfig() is used.
     *    • Future architectures will rely on this injection mechanism by default.
     *****************************************************************************************/
    ...
    //
    //    1) Wi-Fi Guard
    //         • Try reconnecting without blocking
    //         • Detect new Wi-Fi epochs
    //
    //    2) Wi-Fi Milestones
    ...
    //         • Detect new MQTT epochs
    //
    //    4) MQTT Discovery
    //         • Publish Home Assistant discovery payloads once per session
    //
    //    5) MQTT Subscription
    //         • Subscribe to all HA_CONTROL and HA_BUTTON input topics
    //
    //    6) Retained-message Flush
    //         • Avoid old retained messages interfering with the device
    //         • Window duration: mqtt_flush_window (ms)
    //
    //    7) Online/Loss Handling
    //         • Reset communication state
    //         • Feed watchdog continuously
    //
    ...
    void CoreComm() { 
        ...
        if (wifiOK && coreState == CommState::WIFI_NOT_READY) {
            Serial.printf("[HestiaCore::CoreComm] 🌐 New Wi-Fi session ");
            Serial.flush();
        ...
        switch (coreState) {
            ...
            case CommState::MQTT_READY:
                Serial.println(F("[CoreComm] MQTT ready → Waiting for HA_online"));
            ...
            case CommState::HA_ONLINE_CONFIRM:
                Serial.println(F("[CoreComm] HA confirmed online → Starting HA pipeline"));
                Serial.flush();
            ...
            case CommState::START_FLUSH:
                // 1) Activer la logique de flush côté HAIoTBridge
            ...
            case CommState::START_TIMER_FLUSH:
                ...
                Serial.println(F("[HestiaCore::CoreComm | MQTT] 🔭 Starting timer flush..."));
                Serial.flush();
            ...
            case CommState::END_FLUSH:
                // Flush terminé : on peut maintenant attendre HA_online
                FlushState   = false;
            ...
            {       
                Serial.println(F("\n=== [HestiaCore::CoreComm | HAInit ] ✅  HAInit complete. System running. ===\n"));
        ...
        // MQTT loop + watchdog, tant que MQTT reste connecté
    ...
    // =====================================================================================
    //  onMessageReceived — MQTT dispatching to entities
    // =====================================================================================
    ...
    // =====================================================================================
    //  publishToMQTT — Centralized publication with optional logging
    // =====================================================================================
    ...
    // =====================================================================================
    //  logBook — Unified logger (Serial + HA logging)
    // =====================================================================================
